...
if(!class_exists('uwpqsfprocess')){
  ...
  function uajax_result($arg, $id,$pagenumber,$getdata){
	...
	$html = '';
   $customPosts = new WP_Query($arg);
   $var_info = print_r($arg,true);
$comma_separated = implode(" , ", $arg);
$comma_separated2 = implode(" , ", $arg[0]);
//$html .=  "Last SQL-Query: {$customPosts->request}  -- ".$comma_separated." ++ ".$var_info;
	...
	if ( $query->have_posts() ) {
	   ...
	   while ( $query->have_posts() ) {
                ...
                $price=get_product_price($post->ID);
                              
                ;                                
            $html .= '    <div class="col-xs-6 col-xxs-12 col-sm-6 col-sm-4 col-lg-3">
	<div class="thumbnail product-grid">
		<div class="caption">
            			<div class="img-grid-wrapper">
				<a title="'.get_the_title().'" href="'.get_permalink().'">
					<img width="262" height="380" src="'.$url.'" class="img-responsive product-img wp-post-image" alt="'.get_the_title().'">				</a>
			</div>
			...
			<a title="'.get_the_title().'" href="'.get_permalink().'" class="product-title">
			'.get_the_title().'	</a>
		...
		<p class="price">'.$price.'</p>
			...
			<hr class="hidden-xs">
					  ...
					  <div class="col-sm-6">
							<a href="'.get_permalink().'" title="'.get_the_title().'" class="btn btn-detail btn-block">Details</a>					  </div>
					  <div class="col-sm-6">
							<a href="'.get_permalink().'" title="'.get_the_title().'" class="btn btn-buy btn-block" rel="nofollow" target="_blank">Kaufen</a>					  </div>
				 </div>
					</div>
	</div>
</div>';
